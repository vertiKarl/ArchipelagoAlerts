...
export class NameToBeDetermined {
    ...
    private async setupClients(host: string, port: number, slots: string[], password?: string) {
        for(const slot of slots) {
            const client = new Client();
            try {
                console.log("Trying to connect to", `${host}:${port} with slot ${slot} and password ${password}`)
    ...
    private displayAlert() {
        ...
        switch(alert.type) {
            case 'AlertItem':
            {
                ...
                img!.src = image;
                text!.innerHTML = `${alert.slot} received <item class="${item.progression ? "progression" : "item"}">${item.name}</item> from <player style="color: hsl(${stringToHue(item.sender.name)}, 80%, 50%);">${item.sender.name}</player>`
                audio!.src = sound;
            ...
            case "AlertMeta":
            {
                ...
                text.innerHTML = `<player style="hsl(${stringToHue(slot)})">${slot}</player>: ${info}`;
                app.classList.add("hide");
